<template>
    <div class="max-w-2xl w-full m-auto p-6">
        <div>
            <h1 class="text-5xl font-bold">Hi, I'm Adrian</h1>
        </div>
        <div class="flex flex-col">
            <h2 class="py-3 text-2xl">I build things.</h2>
            <div class="">
                <img src="https://i.gifer.com/8Yyg.gif" alt="Anime guy with computer gif"
                    class="w-4/5 h-full rounded float-none m-auto mb-2 xs:float-right xs:ml-3 xs:mb-3 xs:w-45 xs:h-30">
                <p class="font-normal">I create software that's useful, fun, and solves real problems. Whether it's a
                    tool to improve
                    gameplay,
                    an app to enhance learning, or a system to streamline my own projects like YouTube, I love building
                    things that make life easier and more interesting.</p>
            </div>
        </div>

        <div class="flex flex-col">
            <h2 class="py-3 text-2xl">I learn things.</h2>
            <div class="">
                <img src="https://i.pinimg.com/originals/10/d8/ba/10d8ba6db5fbe73ef31375870b481e16.gif"
                    alt="Anime girl at computer gif"
                    class="w-4/5 h-full rounded float-none m-auto mb-2 xs:float-left xs:mr-3 xs:mb-3 xs:w-45 xs:h-30">
                <p class="font-normal">I'm always learning - whether it's new tech, a better way to edit videos, or a
                    fresh approach to
                    problem-solving. Exploring new ideas and skills keeps me sharp, and I enjoy diving into whatever
                    interests me, from software development to content creation.</p>
            </div>
        </div>

        <div class="flex flex-col">
            <h2 class="py-3 text-2xl">I teach things.</h2>
            <div class="">
                <img src="https://i.pinimg.com/originals/a3/28/00/a3280086331589746635e698769c48a2.gif"
                    alt="Anime guy with glasses gif"
                    class="w-3/5 h-full rounded float-none m-auto mb-2 xs:float-right xs:ml-3 xs:mb-3 xs:w-30 xs:h-30">
                <p class="font-normal">I love sharing what I learn. Through my YouTube channel, coaching, and soon,
                    blogging, I break down
                    complex topics to help others improve - whether it's analyzing strategy, understanding tech, or
                    optimizing
                    creative workflows.</p>
            </div>
        </div>

        <router-link to="/about"><button class="btn btn-info">Learn more about me</button></router-link>
    </div>
</template>

<script>
export default {

}
</script>

<style></style>